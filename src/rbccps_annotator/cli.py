...
from rbccps_annotator.segmenter import download_fastsam, status as segmenter_status
...
def main() -> None:
    ...
    yolo = subparsers.add_parser("export-yolo", help="Export reviewed streetlight boxes as a one-class YOLO dataset.")
    ...
    segmenter = subparsers.add_parser("download-segmenter", help="Download/cache FastSAM-s for Magic Surface proposals.")
    ...
    if args.command == "ingest-frames":
        ...
        export_yolo(args.workspace, output, include_candidate_boxes=args.include_candidates)
        ...
        path = download_fastsam(force=args.force)
        print(f"FastSAM segmenter ready: {path.resolve()}")
