...
Contains local dataset storage used for research and experimentation, including raw videos, extracted frames, imported datasets, and derived artifacts. The current organization of this directory is documented in [datasets/README.md](datasets/README.md).
...
This directory does not currently appear to be initialized as a Git repository, but `.gitignore` has been prepared so that the repository structure is ready when Git tracking is enabled.
...
| `datasets/derived/` | Generated artifacts | Preview videos, outputs, and intermediate artifacts | Produced by scripts, experiments, and review workflows | Rebuildable outputs that should not clutter version history |
