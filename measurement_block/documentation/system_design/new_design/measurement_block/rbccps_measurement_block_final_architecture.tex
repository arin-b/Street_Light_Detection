...
\begin{itemize}
  ...
  \item streetlight class label distinguishing lamp-head sources from pole or fixture-support context;
...
The detector class \texttt{streetlight\_lamp\_head} denotes the tight light-source or luminaire box used for status, attribution, and useful-illumination measurement. The detector class \texttt{streetlight\_pole} denotes the visible pole, arm, and fixture-support assembly used for tracking, asset grouping, and map matching. Legacy \texttt{streetlight} labels should be interpreted as lamp-head labels during migration.
...
Streetlight detector labels should separate the tight lamp-head or luminaire source from the visible pole and fixture assembly. The \texttt{streetlight\_lamp\_head} class is the source used by lamp-status, attribution, and affected-region measurement. The \texttt{streetlight\_pole} class is support context for asset grouping, tracking, and map matching; pole tracks must not produce independent useful-illumination reports. When the lamp head is visible but the pole or fixture assembly is not visible enough, the annotation should carry a \texttt{pole\_not\_visible} QA flag rather than a guessed pole box.
