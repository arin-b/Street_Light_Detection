...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build dataset using only Retinex reflectance decomposition."
    )
    ...
    parser.add_argument(
        "--output-root",
        default="datasets/processed/retinex-decomposition"
    )
...
def infer_split(path: Path) -> str:
    ...
    raise ValueError(
        f"Could not infer split from filename: {path.name}"
    )


def matching_label(image_path: Path, label_dir: Path) -> Path:

    label_name = image_path.stem.replace(
        "_images_",
        "_labels_"
    ) + ".txt"
...
def main() -> None:
    ...
    retinex = RetinexDecompositionModel(
        checkpoint=args.retinex_checkpoint,
        enabled=True
    )
    ...
    for index, image_path in enumerate(images, start=1):
        ...
        if (
            ...
            components = retinex.decompose(
                image_path,
                output_dir=retinex_dir,
                device=args.device
            )
            ...
            if (
                ...
                != Image.open(reflectance_path).size
            ):
                raise RuntimeError(
                    f"Reflectance image size changed for {image_path.name}; "
                    "labels are no longer valid."
                )
