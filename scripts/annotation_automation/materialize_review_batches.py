...
DEFAULT_OUTPUT_ROOT = ROOT / "datasets" / "derived" / "annotation_automation" / "reviews" / "batches"
DEFAULT_HARD_NEGATIVE_MANIFEST = (
    ROOT / "datasets" / "derived" / "annotation_automation" / "reviews" / "hard_negative_review_manifest.csv"
)
DEFAULT_CALIBRATION_MANIFEST = (
    ROOT / "datasets" / "derived" / "annotation_automation" / "reviews" / "calibration_subset_manifest.csv"
)
...
def main() -> None:
    ...
    write_index(
        output_root / "hard_negative_batch_index.csv",
        hard_negative_rows,
        fieldnames=list(hard_negative_rows[0].keys()) if hard_negative_rows else [],
    )
    write_index(
        output_root / "calibration_batch_index.csv",
        calibration_rows,
        fieldnames=list(calibration_rows[0].keys()) if calibration_rows else [],
    )
