...
OUTPUT_ROOT = ROOT / "datasets" / "derived" / "annotation_automation"
...
def main() -> None:
    ...
    write_csv(
        output_root / "ambiguous_review_rows.csv",
        ambiguous_rows,
        list(ambiguous_rows[0].keys()) if ambiguous_rows else list(review_rows[0].keys()) if review_rows else [],
    )
    write_csv(
        output_root / "missed_positive_review_rows.csv",
        missed_positive_rows,
        list(missed_positive_rows[0].keys()) if missed_positive_rows else list(review_rows[0].keys()) if review_rows else [],
    )
    write_csv(
        output_root / "pending_review_rows.csv",
        skipped_pending_rows,
        list(skipped_pending_rows[0].keys()) if skipped_pending_rows else list(review_rows[0].keys()) if review_rows else [],
    )
